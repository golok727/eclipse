...
void MovementSystem::Update(ecs::World& world, float deltaTime) {
  world.ForEach<components::Transform, components::PlayerController>(
      [deltaTime](ecs::Entity, components::Transform& transform,
        ...
        if (input::ActionDown(input::Action::MoveLeft)) {
          direction.x -= 1.0f;
        }
        if (input::ActionDown(input::Action::MoveRight)) {
          direction.x += 1.0f;
        }
        if (input::ActionDown(input::Action::MoveUp)) {
          direction.y -= 1.0f;
        }
        if (input::ActionDown(input::Action::MoveDown)) {
          direction.y += 1.0f;
        }
